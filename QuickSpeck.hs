...
main = hspec $ 
  ...
  modifyMaxSuccess (const 200) $ it "zero is the identity element wrt monoSum" $ property
    (\m s n -> monoSum' (m :: Monomial) (monoInit 0 (s :: String) [(n :: Int)]) == monoReduce m)
  modifyMaxSuccess (const 200) $ it "zero is the absorbent element wrt monoProduct" $ property 
    (\m s n -> monoProduct' (m :: Monomial) (monoInit 0 (s :: String) [(n :: Int)]) == (0,[])) 
  modifyMaxSuccess (const 200) $ it "zero is the identity element wrt polySum" $ property
