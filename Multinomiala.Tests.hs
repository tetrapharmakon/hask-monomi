...
main =
  ...
  -- together with monoInit
    it "custom monoInit (fst p) (var p) (exps p) == p" $ do
      monoInit (fst m) (monoExtractVar m) (monoExtractExps m) ==
        m `shouldBe` True
      monoInit (fst a) (monoExtractVar a) (monoExtractExps a) ==
        a `shouldBe` True
      monoInit (fst b) (monoExtractVar b) (monoExtractExps b) ==
        b `shouldBe` True
      monoInit (fst c) (monoExtractVar c) (monoExtractExps c) ==
        c `shouldBe` True
      monoInit (fst d) (monoExtractVar d) (monoExtractExps d) ==
        d `shouldBe` True
      monoInit (fst e) (monoExtractVar e) (monoExtractExps e) ==
        e `shouldBe` True
      monoInit (fst w) (monoExtractVar w) (monoExtractExps w) ==
        w `shouldBe` True
      monoInit (fst v) (monoExtractVar v) (monoExtractExps v) ==
        v `shouldBe` True
  -- monoInit
  ...
  -- monoProduct
    it "monoProduct tested against custom monomials" $ do
      ...
      monoProduct [a, b, c, d, e, f, w, v] `shouldBe`
        (108, [('x', 10), ('y', 8), ('z', 3)])
  -- polyReduce
    ...
    it "polyRaise tested against custom monomials" $ do
      polyRaise [z] 3 `shouldBe` Just []
      polyRaise [z] (-3) `shouldBe` Nothing
      polyRaise [b, c] 2 `shouldBe`
        Just
          (polyReduce
